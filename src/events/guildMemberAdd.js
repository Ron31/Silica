module.exports = async (client, member) => {
    ...
    client.con.query("SELECT * FROM settings WHERE guildid = ?", [member.guild.id], (e, r) => {
        if (!r[0] || r[0].joinchannelid == null) {
            ...
            member.guild.channels.get(r[0].joinchannelid).send(r[0].joinmessage.replace("{user}", `${member.user.tag} [${member}]`).replace("{guild}", member.guild.name));
        }
    });
    client.con.query("SELECT * FROM settings WHERE guildid = ?", [member.guild.id], (e, r) => {
        if (r.length == 0) {
            ...
            member.addRole(r[0].roleid).catch(e => {
