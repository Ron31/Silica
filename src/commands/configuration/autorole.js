                ...
                client.con.query("SELECT * FROM settings WHERE guildid = ?", [message.guild.id], async (e, r) => {
                    ...
                    if (args.join(" ") == "") {
                        if (r.length == 1) {
                            ...
                            return Embeds.success(message.channel, (await client.string(message.guild.id, "command.autorole.currentRole")).replace("$autorole", message.guild.roles.get(r[0].roleid)));
                        } else {
                            return Embeds.notice(message.channel, await client.string(message.guild.id, "command.autorole.roleRequired"));
                        }
                    }
